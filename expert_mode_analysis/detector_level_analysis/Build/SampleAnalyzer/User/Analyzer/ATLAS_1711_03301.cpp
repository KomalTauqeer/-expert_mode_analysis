...
// -----------------------------------------------------------------------------
bool ATLAS_1711_03301::Initialize(const MA5::Configuration& cfg, const std::map<std::string,std::string>& parameters)
{
  // Information on the analysis, authors, ...
  // VERY IMPORTANT FOR DOCUMENTATION, TRACEABILITY, BUG REPORTS
  INFO << "        <><><><><><><><><><><><><><><><><><><><><><><><>" << endmsg;
  INFO << "        <>    Analysis: ATLAS_1711_03301                <>" << endmsg;
  ...
  // Initialize histo
  hist_M = new TH1F("malp_350","",100,0,1000);
...
void ATLAS_1711_03301::Finalize(const SampleFormat& summary, const std::vector<SampleFormat>& files)
{
//For plots, 

TCanvas* c = new TCanvas("c1","");
gPad -> SetLogy();
hist_M -> GetXaxis() -> SetTitle("M_{jj}[GeV]");
hist_M -> GetYaxis() -> SetTitle("Events/Bin");
hist_M -> SetLineWidth(3);
...
bool ATLAS_1711_03301::Execute(SampleFormat& sample, const EventFormat& event)
{
// unchange
